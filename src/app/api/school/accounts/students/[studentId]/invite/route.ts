 ...
 * Creates and issues a cryptographically random, high-entropy invitation token for a student.
 * Required Permission: STUDENT_ACCOUNTS_INVITE
 */
export async function POST(
  ...
  try {
    const auth = await requirePermission(request, {
      permission: 'STUDENT_ACCOUNTS_INVITE',
    });
    ...
    const result = await withTenantContext(schoolId, async (tx) => {
      ...
      if (student.studentUser && student.studentUser.user?.status === 'ACTIVE') {
        return {
          error: 'এই শিক্ষার্থীর ইতিমধ্যে সক্রিয় পোর্টাল অ্যাকাউন্ট রয়েছে।',
          status: 400,
        };
      }

      const inputPayload = {
        recipientPhone: (body as any).recipientPhone || student.phone || '',
        recipientEmail: (body as any).recipientEmail || student.email || undefined,
      ...
      if (!validation.success) {
        return {
          error: 'অবৈধ আমন্ত্রণ তথ্য। শিক্ষার্থীর জন্য সঠিক ফোন নম্বর প্রদান করুন।',
      ...
      // Invalidate existing pending invitations for this student
      ...
      await logAuditEvent({
        ...
        changeSummary: `Created student account invitation for ${student.fullNameEn} [${student.studentCode}] (${recipientPhone})`,
      });
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'শিক্ষার্থী আমন্ত্রণ সফলভাবে তৈরি হয়েছে।',
