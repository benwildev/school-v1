...
export async function GET(request: NextRequest) {
  try {
    ...
    const events = await withTenantContext(schoolId, async () => {
      ...
      return prisma.transportBoardingEvent.findMany({
        where,
        include: {
          student: {
            select: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isBatch) {
      ...
      const createdEvents = await withTenantContext(schoolId, async () => {
        ...
        const records = await Promise.all(
          events.map(async (ev) => {
            ...
            return prisma.transportBoardingEvent.create({
              data: {
                schoolId,
                tripId,
      ...
      const createdEvent = await withTenantContext(schoolId, async () => {
        const trip = await prisma.transportTrip.findFirst({
          where: { id: tripId, schoolId },
        });
        if (!trip) {
